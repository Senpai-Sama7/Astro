...
class AgentEngine:
    ...
    async def register_agent(self, config: AgentConfig, instance: Any = None):
        ...
        await self.db.save_agent_async(config.agent_id, asdict(config), AgentStatus.IDLE.value, config.reliability_score)
    ...
    async def _find_suitable_agent(self, task: Task) -> Optional[str]:
        ...
        for agent_id, config in self.agents.items():
            ...
            active_count = sum(
                1 for task_id, task in self.active_tasks.items()
    ...
    async def _execute_task_with_agent(self, task: Task, agent_id: str):
        ...
        try:
            ...
            if len(self.performance_metrics[agent_id]) > self.max_metrics_per_agent:
                ...
                self.performance_metrics[agent_id] = self.performance_metrics[agent_id][-self.max_metrics_per_agent:]
