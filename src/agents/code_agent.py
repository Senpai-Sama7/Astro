...
class CodeAgent(BaseAgent):
    ...
    async def _generate_code(self, task: Dict[str, Any]) -> TaskResult:
        ...
        try:
            ...
            return TaskResult(
                success=True,
                result_data={'code': code, 'model': self.model_name}
            )
    ...
    async def _execute_code(self, code: str) -> TaskResult:
        ...
        if self.use_docker_sandbox:
            if HAS_DOCKER:
                ...
                logger.warning("Docker unavailable, falling back to local execution (SECURITY RISK)")
    ...
    async def _execute_locally(self, code: str) -> TaskResult:
        ...
        try:
            ...
            try:
                ...
                proc.kill()
                await proc.wait()
                return TaskResult(success=False, error_message="Local execution timed out (30s limit)")
            ...
            return TaskResult(success=False, error_message=f"Local execution failed: {e}")
        finally:
            if temp_path and os.path.exists(temp_path):
                try:
                    os.unlink(temp_path)
    ...
    async def _analyze_code(self, code: str) -> TaskResult:
        ...
        if not code:
            return TaskResult(success=False, error_message="No code provided for analysis.")
